...
public class AuxTransformations {
    ...
    static int[][] subBytes(int[][] state, boolean inverse) {
        ...
        for (int i = 0; i < state.length; i++) {
            for (int j = 0; j < state.length; j++) {
                row = state[i][j] >> 4;
                col = state[i][j] & 15;
                state[i][j] = inverse ? invSBox[row][col] : sBox[row][col];
            }
        }
        return state;
    }
    ...
    static int[][] mixColumns(int[][] state, boolean inverse) {
        ...
        int[][] matrix = inverse ? invMixColumnMatrix : mixColumnMatrix;
        int gf;

        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                for (int k = 0; k < 4; k++) {
                    if (matrix[i][k] == 0 || state[k][j] == 0) {
                        mult[i][j] ^= 0;
                    } else {

                        gf = (L[matrix[i][k] >> 4][matrix[i][k] & 15] +
                                L[state[k][j] >> 4][state[k][j] & 15]) % 255;
                        mult[i][j] ^= E[gf >> 4][gf & 15];
                    }
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.deepToString(prepareMessage(key)));

        int[][] result = addRoundKey(prepareMessage(message), prepareMessage(key));
        ...
        int[][] testColumns = {
                {0x04, 0xd6, 0x00, 0xda12}, //0xD4, 0xBF, 0x5D, 0x30
                {242, 10, 34, 92},
                {1, 1, 1, 1},
                {198, 198, 198, 198}};
        ...
        System.out.println(Arrays.deepToString(mixColumns(transpose(testColumns), true)));
